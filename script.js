// ===== DEVIL REIGN — v7.0 =====
// Website by Hironi | Firebase Edition v7.0
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setTimeout(() => {
    if (!tutorialShown) {
      showToast('Selamat Datang di v7.0! 👋', 'Klik "Lihat Tutorial" untuk panduan pengisian form', 4000);
    } else {
...
async function loadFromFirebase() {
  try {
    ...
    if (settings) { siteSettings = settings; applySettings(settings); }
    ...
    // BUG FIX: Ad popup hanya tampil sekali per hari
...
function applyMaintenanceMode(m) {
  ...
  const themeColors = {
    dark:  { bg:'linear-gradient(135deg,#07070A 0%,#0F0F14 100%)', bar:'rgba(255,255,255,0.15)' },
    blood: { bg:'linear-gradient(135deg,#0A0003 0%,#1A0008 100%)', bar:'rgba(193,18,31,0.5)' },
    blue:  { bg:'linear-gradient(135deg,#020815 0%,#041228 100%)', bar:'rgba(90,200,250,0.4)' },
    warn:  { bg:'linear-gradient(135deg,#080500 0%,#150A00 100%)', bar:'rgba(255,159,10,0.4)' }
  };
  ...
  overlay.innerHTML = `
    ...
    <div style="text-align:center;padding:40px;max-width:480px;width:90%">
      ...
      ${m.eta ? `<div style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:20px;font-size:12px;color:rgba(240,240,245,0.6);margin-bottom:16px">⏱ Estimasi: <strong>${esc(m.eta)}</strong></div>` : ''}
      ...
      <div style="margin-top:28px;font-size:10px;color:rgba(240,240,245,0.2);letter-spacing:2px">DEVIL REIGN v7.0 © ${new Date().getFullYear()}</div>
    </div>
  `;
...
// Console branding
console.log('%c DEVIL REIGN v7.0 ', 'background:linear-gradient(135deg,#C1121F,#7B0010);color:white;font-size:24px;font-weight:bold;padding:10px 20px;border-radius:10px;');
console.log('%c Firebase Edition v7.0 — Bug Fixes + New Features ', 'color:#C1121F;font-size:14px;');
console.log('%c Crafted by Hironi × Upgraded by Claude v7.0 ', 'color:#666;font-size:12px;');
